...
"       possible.
"TexLet g:Tex_CompileRule_pdf = 'pdflatex -interaction=nonstopmode -file-line-error-style $*'

" this was something special, not sure though anymore what it was for
" TexLet g:Tex_CompileRule_pdf = 'mkdir -p build && pdflatex -output-directory=build -interaction=nonstopmode $* && cp *.bib build && cd build && bibtex %:r && cd .. && pdflatex -output-directory=build -interaction=nonstopmode $* && mv build/$*.pdf .'

" default one
" TexLet g:Tex_CompileRule_pdf = 'pdflatex -interaction=nonstopmode $* && bibtex %:r &&  pdflatex -interaction=nonstopmode $* '

" improved bibtex comilation
TexLet g:Tex_CompileRule_pdf = 'pdflatex -interaction=nonstopmode $* && echo $* | cut -d '.' -f 1 | xargs bibtex -- &&  pdflatex --synctex=1 -interaction=nonstopmode $* '
...
if has('win32')
	...
	TexLet g:Tex_ViewRule_dvi = 'yap -l'
elseif has('macunix')
